...
const getBrowserActionDefaults = (
...
export class BrowserActionAPI {
        ...
        case "extension-icon": {
          ...
          if (extension && iconDetails) {
            if (typeof iconDetails.path === "string") {
              const iconAbsPath = resolveExtensionPath(
                extension,
                iconDetails.path
              );
              if (iconAbsPath)
                iconImage = nativeImage.createFromPath(iconAbsPath);
            } else if (typeof iconDetails.path === "object") {
              const imagePath = matchSize(
                iconDetails.path,
                imageSize,
                resizeType
              );
              const iconAbsPath =
                imagePath && resolveExtensionPath(extension, imagePath);
              if (iconAbsPath)
                iconImage = nativeImage.createFromPath(iconAbsPath);
              ...
              const imageData = matchSize(
                iconDetails.imageData as any,
                imageSize,
                resizeType
              );
              iconImage = imageData
  ...
  private getPopupUrl(extensionId: string, tabId: number) {
    ...
    if (!url) {
      try {
        url =
          popupPath &&
          new URL(popupPath, `chrome-extension://${extensionId}`).href;
      } catch {}
    }
  ...
  private getState() {
    ...
    const actions = Array.from(this.actionMap.entries()).map(
      ([id, details]) => {
  ...
  private activate({ sender }: ExtensionEvent, details: ActivateDetails) {
    ...
    switch (eventType) {
      ...
      default:
        console.debug(
          `Ignoring unknown browserAction.activate event '${eventType}'`
        );
    }
  }
  ...
  private activateClick(details: ActivateDetails) {
    ...
    if (this.popup) {
      const toggleExtension =
        !this.popup.isDestroyed() && this.popup.extensionId === extensionId;
      this.popup.destroy();
  ...
  private activateContextMenu(details: ActivateDetails) {
    ...
    const optionsPageUrl = optionsPage
      ? getExtensionUrl(extension, optionsPage)
      : undefined;
